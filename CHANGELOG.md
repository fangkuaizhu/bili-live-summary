...
- **时间线式直播简报（streamer 场景）** — 输出结构从板块分类改为按时间线叙事：
  - 📍 直播概况（两三句概括）
  - 🕐 时间线（主体）：按时间段分段（每段 5-15 分钟，话题变化处更细），段落以【[起始-结束] 小标题】开头，事件锚定具体时间戳 `[mm:ss]`
  - 🔑 关键角色与术语（标注阵营/类型 + 首次提及语境）
  - 💥 高光时刻（按时间列出 3-12 个）
  - 🎙️ 氛围与观众互动
  - 设计参考 transcript-critic 的时间戳锚定理念（每个观察可回溯验证）
- **领域术语表注入** — 新增 `SCENE_TERMS` / `get_scene_terms()`：纠错与总结的 prompt 均注入领域专有名词（默认含第五人格全角色表：求生者 50+ / 监管者 35+ / 术语）。解决 Whisper 同音错字在领域语境下无法纠正的问题（如「新手」→「心兽」新监管者、「魚人」→「渔女」）。可经 config.local.json `scene_terms.<scene>` 覆盖。

### 🐛 Bug Fixes

- **纠错对领域同音词过度保守** — 原“不确定时保留原文”规则导致领域专有名词（心兽/渔女等）同音错字被保留；新增例外规则：当转写词与术语表同音/近音且处于专业语境（角色强度、抽卡、皮肤、技能）时，必须改写为术语表正确写法。实测角色强度讨论区 6 处全部纠正。

### 🔧 Internal

- `corrector.py`：`CORRECTION_PROMPT` 改为模板 `CORRECTION_PROMPT_TEMPLATE`，`_call_*_correction` 新增 scene 参数注入术语表。
- `summarizer.py`：新增 `_with_terms()`，段总结/合并/单段总结统一注入术语表。
